...
export default function NFTCard({ item }) {
  ...
  return (
    ...
    <NFTCardEl>
      <Card>
      <ItemImage>
          <Image src={image} width="1024" height="1025" />
        </ItemImage>
        <InfoSection>
        <TSection>
            
            <StockEl>Cyber Cubs</StockEl>
          </TSection>
         
          <ItemTitle>{name}</ItemTitle>
          <PriceSection>Listed for {price} <Image src={fren} width="22" height="10" /> FREN</PriceSection>
          <BottomSection>
           
            <AuthorEl>Owned buy : {owner.slice(0,6)}...{owner.slice(owner.length-4)}</AuthorEl>
            <LikesEl>
              <Button >Offer</Button>
            </LikesEl>
